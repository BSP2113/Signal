#!/usr/bin/env python3
"""Re-validation Phase B2 — RESUME after the 2026-05-22 SSH disconnect.

The original run (revalidate_logic.py) completed the sanity check and the
confirm_bar config before it was cut off. This script reuses those two
finished scratch files and re-runs only the remaining 6 logic rules, then
prints the full per-rule value table.

Leave-one-out: each rule flipped OFF in turn (every other rule shipped),
fixed sim 2026-03-02..05-21. Value of rule = all-shipped - rule-off.
...
# confirm_bar already finished cleanly before the disconnect — reuse it.
REMAINING = ["two_bar_trail", "take_no_cap", "no_progress",
             "early_weak", "pre10_take_block", "post11_take_block"]

results = {}

# --- reuse the two finished scratch files ---------------------------------
for tag in ("sanity", "confirm_bar"):
    spath = os.path.join(SIGNAL_DIR, f"scratch_revb2_{tag}.json")
    data = [x for x in json.load(open(spath)) if "Exercise 1" in x["title"]]
    name = "ALL SHIPPED (sanity check)" if tag == "sanity" else "confirm_bar OFF"
    results[name] = sum(x["total_pnl"] for x in data)
    print(f"reused {tag}: {len(data)} days  total ${results[name]:+,.2f}", flush=True)

# --- re-run the 6 interrupted / unstarted configs -------------------------
for off in REMAINING:
    for k in FLAGS:
        ex.REVAL_FLAGS[k] = True
    ex.REVAL_FLAGS[off] = False
    scratch = f"scratch_revb2_{off}.json"
    spath = os.path.join(SIGNAL_DIR, scratch)
    if os.path.exists(spath):
        os.remove(spath)            # clears the partial two_bar_trail file
    name = f"{off} OFF"
...
# --- final table ----------------------------------------------------------
print("\n" + "=" * 70)
print("RE-VALIDATION B2 — logic-driven shipped rules (fixed sim)")
